...
import { borderColor, darkGrayColor, primaryColor } from "@/style/color";
...
export default function Home() {
  ...
  const [tab, setTab] = useState("アイテム");
  ...
  const removeItem = (id: number) => {
    ...
    if (index >= 0) {
      ...
      p.quantity -= 1;
      if (p.quantity <= 0) {
        newOrder.order_items.splice(index, 1);
      }
    }
    setOrder(newOrder);
  };
    ...
    <main>
      <Stack direction={"row"}>
        <Stack
          ...
          overflow={"scroll"}
        >
          <Header selected={tab} setSelected={setTab} />
          {/* make responsive item grid using flexbox */}
          {tab == "アイテム" && (
            <Grid container spacing={2}>
              {products.map((product) => (
                <Grid item sm={6} md={4} lg={3} key={product.id}>
                  <Stack component={"a"} onClick={() => addItem(product.id)}>
                    <img
          ...
          <Stack display={tab == "スキャン" ? "flex" : "none"} gap={2}>
